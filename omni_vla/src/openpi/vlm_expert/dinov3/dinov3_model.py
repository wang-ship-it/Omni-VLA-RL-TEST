...
class DINOv3ViTAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        BP, _ = hidden_states.size()
        patches = BP // batch_size
        ...
        query_states = query_states.transpose(1, 2).reshape(batch_size*patches, self.num_heads, self.head_dim)
        key_states = key_states.transpose(1, 2).reshape(batch_size*patches, self.num_heads, self.head_dim)
        value_states = value_states.transpose(1, 2).reshape(batch_size*patches, self.num_heads, self.head_dim)
        ...
        outputs = context_layer.reshape(batch_size*patches, -1)
...
class DINOv3ViTModel(DINOv3ViTPreTrainedModel):
    ...
    def forward(
        ...
        bool_masked_pos (`torch.BoolTensor` of shape `(batch_size, sequence_length)`):
            ...
            pre-training.
        """
        # print("self.embeddings.patch_embeddings.weight.dtype", self.embeddings.patch_embeddings.weight.dtype)
        pixel_values = pixel_values.to(self.embeddings.patch_embeddings.weight.dtype) #float32
        hidden_states = self.embeddings(pixel_values)
        position_embeddings = self.rope_embeddings(pixel_values)
        
        B, S, D = hidden_states.size()
        # before torch.Size([48, 201, 1280])
