...
__HIGH_QUALITY_DATASETS__ = ['BlinkVision', 'Game', 'GameNew', 'DynamicStereo', 'FlyingThings3D', 'gtasfm', 'hypersim', 'matrixcity', 'MidAir', 'Monkaa', 'PointOdyssey', 'Sintel', 'Spring', 'tartanair', 'unreal4k', 'VirtualKitti', 'nav_20w']
__MIDDLE_QUALITY_DATASETS__ = ['BlendedMVG', 'blendedmvs', 'DTU', 'ETH3D', 'scannet', 'scannetpp', 'taskonomy', "dl3dv", "omniworld",  "mapfree", "ariasyn", "arkit_high"]
__INDOOR_DATASETS__ = ['hypersim', 'scannet', 'scannetpp', 'taskonomy', 'arkitscenes', 'nav_20w']
...
class PointLoss(nn.Module):
    ...
    def prepare_segformer(self):
        ...
        self.segformer.load_state_dict(torch.load('segformer.b0.512x512.ade.160k.pth', map_location=torch.device('cpu'), weights_only=False)['state_dict'])
...
class Pi3Loss(nn.Module):
    ...
    def prepare_gt(self, gt):
        ...
        # dataset_names = gt[0]['view_infos'].split('/')[0]
        dataset_names = []
        # print("len(gt['view_infos'])", len(gt['view_infos']), 'N::', N) #16, 16
        for d in range(0, len(gt['view_infos']), N):
            dataset_names.append(gt['view_infos'][d].split('/')[0])
        ...
        if valid_batch.sum() > 0:
            ...
            norm_factor = all_dis.sum(dim=[-1, -2]) / (masks[valid_batch].float().sum(dim=[-1, -2, -3]) + 1e-8).to(torch.bfloat16) 

            gt_pts[valid_batch] = gt_pts[valid_batch] / norm_factor[..., None, None, None, None].to(torch.bfloat16)
            poses[valid_batch, ..., :3, 3] /= norm_factor[..., None, None].to(torch.bfloat16)
