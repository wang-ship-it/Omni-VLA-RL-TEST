...
class ParquetStandardIterableDataset(DistributedIterableDataset):
    ...
    def __init__(
        self, dataset_name, dino_transform, tokenizer, vit_transform, 
        data_dir_list, num_used_data, parquet_info,
        local_rank=0, world_size=1, num_workers=8, data_status=None, shuffle_seed=0, 
    ):
        """
        ...
        for data_dir, num_data_path in zip(data_dir_list, num_used_data):
            ...
            for data_path in data_paths:
                if data_path in parquet_info.keys(): 
                    num_row_groups = parquet_info[data_path]['num_row_groups'] # = 10 
                    for rg_idx in range(num_row_groups):
                        row_groups.append((data_path, rg_idx))  
        self.rng.seed(0)
        self.rng.shuffle(row_groups)
        return row_groups
            ...
            for global_row_group_idx, (parquet_file_path, row_group_id) in enumerate(
                ...
                with fs.open_input_file(parquet_file_path) as f:
                    try:
                        ...
                        print(f'Open file Error {e} in rg#{row_group_id}, {parquet_file_path}')
                        continue
                    ...
                    for row_idx, row in df.iterrows():
                        try:
                            ...
                            data['parquet_info'] = {
                                ...
                                "row_idx": int(row_idx),
                                "global_row_group_idx": int(global_row_group_idx),
